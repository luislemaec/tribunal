-- Normaliza exclusivamente documentos historicos S/N. La identidad funcional
-- resultante se deriva de la PK y no representa una cedula ecuatoriana.
--
-- Tabla real:    public.tb_persona
-- PK real:       pers_id (INTEGER)
-- Documento:     pers_documento (VARCHAR(255))
...
BEGIN
    ...
    SELECT COUNT(*),
     ...
     WHERE BTRIM(p.pers_documento) = 'S/N';
    ...
    -- Aunque actualmente no existe UNIQUE sobre pers_documento, esta
    -- validacion evita colisionar con un SN-{id} ya asignado a otra persona.
    IF EXISTS (
        SELECT 1
         ...
         WHERE BTRIM(origen.pers_documento) = 'S/N'
    ) THEN
        RAISE EXCEPTION
    ...
    IF v_restantes <> 0 THEN
        RAISE EXCEPTION
            'Persisten % personas con documento S/N despues de la normalizacion.',
            v_restantes;
    END IF;
    ...
    RAISE NOTICE
        'Personas con S/N antes: %, actualizadas: %, restantes: %.',
