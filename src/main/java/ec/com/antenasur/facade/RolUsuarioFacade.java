...
public class RolUsuarioFacade extends AbstractFacade<RolUsuario, Integer> {
	...
	public List<RolUsuario> findByUserNameAndRoleName2(String userName, String roleName) {
		try {
			String hql = "select ru from RolUsuario ru join fetch ru.rol join fetch ru.usuario "
					+ "where ru.usuario.estado=true " + "and ru.usuario.username = :userName "
					+ "and ru.estado = true and ru.rol.estado = true "
	...
	public List<RolUsuario> getRolesUsuariosActivos(List<Rol> listaRoles) {
		try {
			String sql = "select ru from RolUsuario ru " + "INNER JOIN FETCH ru.usuario u "
					+ "INNER JOIN FETCH ru.rol r "
					+ "where u.estado=true and ru.estado=true and r.estado=true and r IN :listaRoles  ";
			Query query = super.getEntityManager().createQuery(sql);
	...
	/**
	 * Retorna las relaciones de rol de un usuario, incluidas las inactivas. La baja
	 * de un usuario es lógica, por lo que estas relaciones se reutilizan al
	 * reactivar la cuenta y se evita violar {@code uk_rol_usuario}.
	 */
	...
	/**
	 * Busca solo la página solicitada por la tabla de usuarios. Las asociaciones
	 * necesarias para el DTO se cargan en la misma consulta para evitar consultas
	...
	public List<RolUsuario> buscarUsuarios(FiltroUsuarioDTO filtro, int primerRegistro, int tamanoPagina) {
		...
		String hql = "SELECT ru FROM RolUsuario ru " + "JOIN FETCH ru.usuario u " + "JOIN FETCH ru.rol r "
				+ "LEFT JOIN FETCH u.personsa p " + "LEFT JOIN FETCH u.iglesia i " + construirWhereBusqueda(filtro)
	...
	public int contarUsuarios(FiltroUsuarioDTO filtro) {
		Session session = super.getEntityManager().unwrap(Session.class);
		Filter filtroActivo = deshabilitarFiltroActivoSiCorresponde(session, filtro);
		String hql = "SELECT COUNT(ru.id) FROM RolUsuario ru " + "JOIN ru.usuario u " + "JOIN ru.rol r "
				+ "LEFT JOIN u.personsa p " + "LEFT JOIN u.iglesia i " + construirWhereBusqueda(filtro);
		try {
	...
	private String construirWhereBusqueda(FiltroUsuarioDTO filtro) {
		...
		agregarFiltroTexto(condiciones, "u.username", valorFiltro(filtro != null ? filtro.getUsername() : null),
				"username");
		agregarFiltroTexto(condiciones, "p.nombres", valorFiltro(filtro != null ? filtro.getNombres() : null),
				"nombres");
		agregarFiltroTexto(condiciones, "i.nombre", valorFiltro(filtro != null ? filtro.getIglesiaNombre() : null),
				"iglesia");
