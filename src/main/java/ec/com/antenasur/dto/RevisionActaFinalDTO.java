...
public class RevisionActaFinalDTO implements Serializable {
	...
	public long getBlancos() {
		return sumar("BLANCOS");
	}
	...
	public Long getDiferenciaTotal() {
		return totalDeclarado == null ? null : totalDeclarado.longValue() - getTotal();
	}

	public boolean isCuadrada() {
		if (resultados == null || resultados.isEmpty() || validosDeclarados == null || totalDeclarado == null
				|| validosDeclarados < 0 || totalDeclarado < 0)
			return false;
		...
		for (var r : resultados) {
			if (r == null || r.getCategoriaId() == null || !categorias.add(r.getCategoriaId())
					|| r.getTotalVotos() == null || r.getTotalVotos() < 0 || clasificar(r) == null)
				return false;
	...
	public static String clasificar(EscrutinioDTO r) {
		if ("LISTA".equals(r.getCategoriaTipo()))
			return "LISTA";
		String nombre = r.getCategoriaNombre() == null ? ""
				: r.getCategoriaNombre().trim().toUpperCase(java.util.Locale.ROOT);
		if ("BLANCOS".equals(nombre) || "NULOS".equals(nombre))
			return nombre;
		if ("PAPELETAS RESTANTES".equals(nombre) || "PAPELTAS RESTANTES".equals(nombre))
			return "PAPELETAS";
		if ("LEGACY".equals(r.getCategoriaTipo()) && nombre.matches("LISTA \\d+"))
			return "LISTA";
		return null;
	}
}
