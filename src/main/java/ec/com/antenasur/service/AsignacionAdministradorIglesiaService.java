...
public class AsignacionAdministradorIglesiaService {
	...
	public DiagnosticoAsignacionAdministradorDTO previsualizarAsignacion(Integer iglesiaId, Integer iglesiaPersonaId) {
	...
	public ResultadoProvisionUsuarioDTO asignar(Integer iglesiaId, Integer iglesiaPersonaId, String correo) {
		try {
			log.info("Asignacion IglesiaAdmin iniciada. iglesiaId={}, iglesiaPersonaId={}", iglesiaId,
					iglesiaPersonaId);
			...
			List<IglesiaPersona> relaciones = iglesiaPersonaFacade.listarActivasPorDocumento(diagnostico.getCedula(),
					true);
			IglesiaPersona relacion = relaciones.stream()
					.filter(item -> iglesiaPersonaId.equals(item.getId()) && item.getPersona() != null
					...
					.findFirst().orElseThrow(() -> new NegocioException("iglesias.admin.error.persona.no.pertenece"));
			...
			if (adminActual != null && !relacion.getPersona().getId().equals(adminActual.getPersonaId())) {
				log.info("Reasignando IglesiaAdmin. iglesiaId={}, usuarioAnteriorId={}, personaNuevaId={}", iglesiaId,
						adminActual.getId(), relacion.getPersona().getId());
				usuarioService.removerAdminDeIglesia(iglesiaId);
			}
			ResultadoProvisionUsuarioDTO resultado = usuarioService
					.provisionarAdministradorIglesiaExclusivo(relacion.getPersona(), correo, rol, iglesiaId);
			log.info(
					"Asignacion IglesiaAdmin completada. iglesiaId={}, personaId={}, usuarioId={}, rol={}, reutilizado={}, reactivado={}",
					iglesiaId, relacion.getPersona().getId(), resultado.getUsuario().getId(), rol.getNombre(),
					resultado.isReutilizado(), resultado.isReactivado());
			return resultado;
		} catch (RuntimeException e) {
			sessionContext.setRollbackOnly();
			log.warn("Asignacion IglesiaAdmin revertida. iglesiaId={}, iglesiaPersonaId={}, motivo={}", iglesiaId,
					iglesiaPersonaId, e.getMessage());
			throw e;
		}
	}
	...
	private IglesiaPersona validarAsignacionBase(Integer iglesiaId, Integer iglesiaPersonaId) {
		...
		if (relacion == null || !Boolean.TRUE.equals(relacion.getEstado()) || relacion.getIglesia() == null
				|| !iglesiaId.equals(relacion.getIglesia().getId()) || relacion.getPersona() == null
				|| !Boolean.TRUE.equals(relacion.getPersona().getEstado())
	...
	private DiagnosticoAsignacionAdministradorDTO construirDiagnostico(IglesiaPersona relacion, boolean bloquear) {
		...
		var usuario = usuarioService.obtenerUsuarioPorPersonaIncluyendoInactivos(relacion.getPersona().getId());
		if (usuario != null && Boolean.TRUE.equals(usuario.getEstado()) && usuario.getIglesiaNombre() != null
				&& !usuario.getIglesiaNombre().isBlank()) {
			iglesias.add(usuario.getIglesiaNombre());
		}
		diagnostico.setIglesiasAsociadas(List.copyOf(iglesias));
		diagnostico.setPersonasDuplicadas(personas.stream().map(this::identificarPersona).toList());
		return diagnostico;
	}

	private String identificarPersona(Persona persona) {
		String nombres = persona.getNombres() == null || persona.getNombres().isBlank() ? "Sin nombres registrados"
				: persona.getNombres().trim();
		return "ID " + persona.getId() + " — " + nombres;
	}

	/**
	 * Mantiene auditoría e historial mediante bajas lógicas de los vínculos
	 * alternos.
	 */
