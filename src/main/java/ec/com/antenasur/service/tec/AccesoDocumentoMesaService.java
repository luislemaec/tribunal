...
/** La identidad y el alcance se resuelven desde Elytron, nunca desde parametros JSF. */
@Stateless
...
public class AccesoDocumentoMesaService {
    ...
    @Inject private MiembroJRVService miembroService;
    ...
    public String usuarioActual() { return contexto.getCallerPrincipal().getName(); }
    ...
    public Integer mesaPermitida(Integer procesoId) {
        ...
        if (contexto.isCallerInRole("SITEC-Presidente-mesa")) {
            ...
            var designacion = usuario == null || usuario.getPersonsa() == null ? null
                    : miembroService.obtenerDesignacionPresidentePorPersonaProceso(usuario.getPersonsa().getId(), procesoId);
            if (designacion != null && designacion.getMesa() != null) return designacion.getMesa().getId();
        } else if (esRevisor()) {
            return null;
        }
    ...
    public void exigirRevisor(Integer mesaId, Integer procesoId) {
        ...
        if (!esRevisor()) throw new NegocioException(Constantes.getMensaje("reportesMesa.certificados.no.autorizado"));
    }
}
