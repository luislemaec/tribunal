...
@ec.com.antenasur.security.menu.AccesoPagina({ "permisos" })
...
public class MenuRolService extends AbstractService<MenuRol, Integer, MenuRolFacade> {
	...
	@jakarta.annotation.security.RolesAllowed({ "SITEC-Administrador", "SITEC-Tribunal", "SITEC-IglesiaAdmin",
			"SITEC-Presidente-mesa" })
	public MenuRol delete(MenuRol entity) {
	...
	@jakarta.annotation.security.RolesAllowed({ "SITEC-Administrador", "SITEC-Tribunal", "SITEC-IglesiaAdmin",
			"SITEC-Presidente-mesa" })
	public void remove(MenuRol entity) {
	...
	@jakarta.annotation.security.RolesAllowed({ "SITEC-Administrador", "SITEC-Tribunal", "SITEC-IglesiaAdmin",
			"SITEC-Presidente-mesa" })
	public List<MenuRol> findRange(int[] range) {
	...
	/**
	 * Devuelve la asignación menu-rol existente, o un placeholder transitorio (sin
	 * id, {@code estado=false}) si todavía no existe. Útil para construir árboles
	 * de permisos donde cada nodo necesita un MenuRol — existente o no — para el
	 * binding del checkbox.
	 *
	...
	/**
	 * Versión DTO de {@link #obtenerOPrepararPorMenuYRol(Menu, Rol)}: recibe ids,
	 * retorna un MenuRolDTO existente o un placeholder transitorio (sin id,
	 * estado=false).
	 */
	public MenuRolDTO obtenerOPrepararDTOPorMenuYRolIds(Integer menuId, Integer rolId) {
		if (menuId == null || rolId == null)
			return null;
	...
	/**
	 * Persiste el toggle de un permiso menu-rol. Si el DTO trae id, hace edit
	 * (typical flow al cambiar el estado del checkbox); si no, crea con el estado
	 * actual.
	 */
	public MenuRolDTO guardarDesdeDTO(MenuRolDTO dto) {
		if (dto == null || dto.getMenu() == null || dto.getRol() == null)
			return null;
	...
	private List<MenuRolDTO> mapearLista(List<MenuRol> entidades) {
		...
		if (entidades == null)
			return resultado;
		for (MenuRol mr : entidades)
			resultado.add(MenuRolDTO.fromEntity(mr));
		return resultado;
	}
}
