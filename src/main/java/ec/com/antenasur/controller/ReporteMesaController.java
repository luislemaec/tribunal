...
public class ReporteMesaController implements Serializable {
    ...
    @Inject private DocumentoBean documentoBean;
    @Inject private ActaFisicaEscrutinioService actaFisicaEscrutinioService;
    @Inject private ec.com.antenasur.service.tec.DisponibilidadDocumentoMesaService disponibilidadDocumental;
    ...
    @Getter private DocumentoDTO actaFisicaVisualizada;
    @Getter private List<EscrutinioDTO> resultadosActaFinal = new ArrayList<>();
    @Getter private ec.com.antenasur.dto.RevisionActaFinalDTO revisionActaFinal = new ec.com.antenasur.dto.RevisionActaFinalDTO();
    @Getter private List<ec.com.antenasur.dto.MiembroJRVDTO> juntaVisualizada = new ArrayList<>();
    @Getter private String motivoActaFinal;
    private byte[] contenidoActaFisica;
    private boolean puedeValidarActaFinal;
    @Getter private Integer mesaActaFinalId;
    @Getter private Integer recintoActaFinalId;
    ...
    @Getter private List<MesaDocumentosDTO> resumenesDocumentales = new ArrayList<>();
    @Getter private List<OpcionPadronDTO> cantonesDocumentales = new ArrayList<>();
    @Getter private List<OpcionPadronDTO> parroquiasDocumentales = new ArrayList<>();
    @Getter @Setter private Integer cantonDocumentalId;
    @Getter @Setter private Integer parroquiaDocumentalId;
    @Getter @Setter private String estadoDocumental;
    @Getter @Setter private String busquedaDocumental;
    @Getter @Setter private int primeraFilaDocumental;
    ...
    public void generarDocumentoMesa(Integer mesa, Integer recinto, String tipo, boolean regenerar) {
        try {
            ...
            procesoBean.okActivityRegister((regenerar ? "REGENERA " : "GENERA ") + tipoDocumento.name(), documento.getCodigo());
            actualizarResumenDocumental(mesa);
            JsfUtil.addSuccessMessage(Constantes.getMensaje(regenerar
                    ? "reportesMesa.documento.regenerado" : "reportesMesa.documento.generado"));
    ...
    public StreamedContent descargarDocumentoMesa(Integer mesa, Integer recinto, String tipo) {
        try {
            DocumentoDTO documento = reporteMesaService.obtenerDocumentoActivo(TipoDocumentoMesa.valueOf(tipo), procesoId,
                    recinto, mesa, personaId(), presidenteRestringido);
    ...
    public void visualizarActaFisica(Integer mesa, Integer recinto) {
        ...
        try {
            actaFisicaVisualizada = reporteMesaService.obtenerDocumentoActivo(
                    TipoDocumentoMesa.ACTA_FISICA_ESCRUTINIO, procesoId, recinto, mesa,
                    personaId(), presidenteRestringido);
            ReporteMesaDTO datos = reporteMesaService.consultar(procesoId, recinto, mesa,
                    personaId(), presidenteRestringido);
            resultadosActaFinal = datos != null && datos.getEscrutinios() != null
                    ? new ArrayList<>(datos.getEscrutinios()) : new ArrayList<>();
            ...
            if (imagen == null) throw new NegocioException(Constantes.getMensaje("reportesMesa.actaFisica.error.visualizar"));
            ...
            try {
                ...
                puedeValidarActaFinal = !ActaFisicaEscrutinioService.VALIDADA.equals(actaFisicaVisualizada.getEstadoRevision());
            } catch (NegocioException e) {
    ...
    /** Renderiza la instantanea autorizada sin retener entidades ni consultar JPA desde un getter. */
    public StreamedContent getImagenActaFisica() {
        if (contenidoActaFisica == null) return null;
    ...
    public void guardarActaFinal() {
        try {
            ...
            actaFisicaEscrutinioService.validarActaFinal(actaFisicaVisualizada.getId(), mesaActaFinalId,
                    procesoId, revisionActaFinal);
    ...
    public void actualizarDisponibilidadDocumental() {
        ...
        cargarResumenesDocumentales();
        if (primera < resumenesDocumentales.size()) primeraFilaDocumental = primera;
    }
    ...
    private void cargarResumenesDocumentales() {
        ...
        try {
            resumenesDocumentales = new ArrayList<>(reporteMesaService.listarResumenDocumentos(procesoId,
                    null, null, personaId(), presidenteRestringido));
            cargarOpcionesDocumentales();
            resumenesDocumentales.removeIf(fila -> (cantonDocumentalId != null && !cantonDocumentalId.equals(fila.getCantonId()))
            ...
            if (busquedaDocumental != null && !busquedaDocumental.isBlank()) {
                ...
                resumenesDocumentales.removeIf(fila -> !(texto(fila.getRecinto()).contains(texto)
                        || texto(fila.getMesa()).contains(texto)));
            }
        } catch (NegocioException e) {
            resumenesDocumentales = new ArrayList<>();
            JsfUtil.addWarningMessage(e.getMessage());
    ...
    private void actualizarResumenDocumental(Integer mesaId) {
        MesaDocumentosDTO actualizado = reporteMesaService.consultarResumenDocumento(procesoId, mesaId,
                personaId(), presidenteRestringido);
        ...
        for (int i = 0; i < resumenesDocumentales.size(); i++) {
            if (mesaId.equals(resumenesDocumentales.get(i).getMesaId())) {
                ...
                else resumenesDocumentales.set(i, actualizado);
                if (primeraFilaDocumental >= resumenesDocumentales.size()) primeraFilaDocumental = 0;
                return;
            }
        }
    }
    ...
    private void cargarOpcionesDocumentales() {
        ...
        for (MesaDocumentosDTO fila : resumenesDocumentales) {
            if (fila.getCantonId() != null) cantones.put(fila.getCantonId(), fila.getCanton());
            if (fila.getParroquiaId() != null && (cantonDocumentalId == null || cantonDocumentalId.equals(fila.getCantonId())))
                parroquias.put(fila.getParroquiaId(), fila.getParroquia());
        }
        cantonesDocumentales = cantones.entrySet().stream().map(e -> new OpcionPadronDTO(e.getKey(), e.getValue())).toList();
        parroquiasDocumentales = parroquias.entrySet().stream().map(e -> new OpcionPadronDTO(e.getKey(), e.getValue())).toList();
    }
